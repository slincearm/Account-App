...
### UI/UX 介面改善
- **群組詳情頁面卡片合併**
  - 將「成員列表」與「各類別支出」合併為單一卡片，提升介面簡潔度
  - **成員列表**：置於卡片頂部，且改為常駐顯示，不再受下方折疊功能影響
  - **視覺優化**：移除「成員」與「各類別支出」的標題，減少視覺干擾，專注於內容呈現
  - 新增分隔線區隔成員列表與支出資訊

### 程式碼品質與穩定性
- **Lint 設定修正**
  - 修正 ESLint 設定，排除 `functions/` 目錄，解決根目錄與 Functions 目錄的 CommonJS/ESM 模組規範衝突
  - `npm run lint` 現可正確執行並通過檢查
- **記憶體安全性提升**
  - 優化 `useGroup` Hook
  - 新增 `isMounted` 檢查機制，防止在非同步操作（如獲取成員資料）完成前組件已卸載導致的狀態更新警告

### 影響檔案
- `src/pages/GroupDetail.tsx` - 卡片合併與標題移除
- `src/hooks/useGroup.ts` - 實作 `isMounted` 安全檢查
- `eslint.config.js` - 新增忽略規則

## [2025-12-30] - 費用顯示優化與輸入體驗增強

### 費用列表顯示優化
- **多行式版面設計**
  - 將費用項目資訊從單行改為多行垂直排列，提升閱讀性
  - 順序調整為：
    1. 費用名稱 (Description)
    2. 類別 (Category) - 顯示對應顏色
    3. 付款人 (Payer)
    4. 分攤金額/人數 (Split Info)
- **視覺呈現調整**
  - 縮小輔助資訊字體至 `0.75rem`
  - 設定 `white-space: nowrap` 避免文字換行導致版面錯位

### 金額輸入體驗增強
- **數字鍵盤優化**
  - 設定 `inputMode`，在行動裝置上點擊金額欄位時自動彈出數字鍵盤
- **幣別小數點規則**
  - **美金 (USD) / 歐元 (EUR)**：支援 2 位小數 (如 10.00)
  - **人民幣 (CNY)**：支援 1 位小數 (如 10.0)
  - **台幣 (TWD) 及其他**：僅支援整數
  - 依據選擇的幣別自動調整 `step` 屬性與格式化邏輯

### 影響檔案
- `src/pages/GroupDetail.tsx` - 費用列表版面重構
- `src/components/AddExpenseModal.tsx` - 金額輸入邏輯與字體調整

## [2024-12-26] - 群組詳情頁面折疊功能實現

### UI/UX 功能增強
- **成員卡片展開/收合**
  - 成員卡片預設為收合狀態，僅顯示標題和成員人數
  - 支援點擊展開查看完整成員列表
  - 提升頁面整潔度，減少初始視覺負擔

- **支出分類圖表展開/收合**
  - 支出分類卡片預設為收合狀態，僅顯示總支出金額
  - 點擊總支出區域可展開查看分類詳情、圖表和結算摘要
  - 實現更好的用戶交互體驗，頁面載入更快
...
### 影響檔案
- `src/pages/GroupDetail.tsx` - 實現成員卡片和支出分類卡片的展開/收合功能
